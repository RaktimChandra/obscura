    ...
    const draw = () => {
      ...
      figs.current.forEach(f => {
        ...
        if (exposed) {
          ...
          // OBSCURA: redaction bar over the head, skeleton dots only
    ...
    <div className="reveal-wrap">
      ...
      <div className="reveal-foot">
        <div className="expose">
          <span><span className="lbl">RAW EXPOSED</span><br /><span className="num raw">{exposed}</span></span>
          <span><span className="lbl">OBSCURA EXPOSED</span><br /><span className="num safe">0</span></span>
        </div>
        <button className="btn ghost" onClick={() => setReal(r => !r)}>
